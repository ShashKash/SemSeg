...
def conv_conv_pool(input, num_filters, filter_size, pool_size, block_nos):
    with tf.variable_scope(f"block{block_nos}", reuse=tf.AUTO_REUSE):
        W1 = tf.get_variable("w_conv1", [filter_size,filter_size,input.shape[3],num_filters], initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        W2 = tf.get_variable("w_conv2", [filter_size,filter_size,num_filters,num_filters], initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        b1 = tf.get_variable("bias1", [num_filters], initializer=tf.constant_initializer(0.01))
        b2 = tf.get_variable("bias2", [num_filters], initializer=tf.constant_initializer(0.01))

        conv_block = tf.nn.conv2d(input, filter=W1, strides=[1,1,1,1], padding="VALID")
        conv_block = tf.nn.bias_add(conv_block, b1)
        conv_block = tf.nn.relu(conv_block)
        conv_block = tf.nn.conv2d(conv_block, filter=W2, strides=[1,1,1,1], padding="VALID")
        conv_block = tf.nn.bias_add(conv_block, b2)
        conv_block = tf.nn.relu(conv_block)

        pooled_block = tf.nn.max_pool(conv_block, ksize=[1,pool_size,pool_size,1], strides=[1,pool_size,pool_size,1], padding="VALID")
    return conv_block, pooled_block

def conv_upconv(input, num_filters, conv_filter_size, upconv_scale, block_nos):
    with tf.variable_scope(f"block{block_nos}", reuse=tf.AUTO_REUSE):
        W_conv = tf.get_variable("w_conv", [conv_filter_size,conv_filter_size,input.shape[3],num_filters], initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        W_upconv = tf.get_variable("w_upconv", [upconv_scale,upconv_scale,num_filters//2,num_filters], initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        b1 = tf.get_variable("bias1", [num_filters], initializer=tf.constant_initializer(0.01))
        ...
        output_shape = tf.stack([conv_block.shape[0], conv_block.shape[1]*upconv_scale, conv_block.shape[2]*upconv_scale, conv_block.shape[3]//2])
        b2 = tf.get_variable("bias2", [conv_block.shape[3]//2], initializer=tf.constant_initializer(0.01))

        conv_block = tf.nn.conv2d_transpose(conv_block, W_upconv, output_shape, strides=[1,2,2,1], padding="VALID")
        conv_block = tf.nn.bias_add(conv_block, b2)
        conv_block = tf.nn.relu(conv_block)
...
def concat_conv(prev_conved, upconved, num_filters, filter_size, block_nos):
    with tf.variable_scope(f"block{block_nos}", reuse=tf.AUTO_REUSE):
        ...
        offsets = tf.stack([0, (prev_conved.shape[1]-target_height)//2, (prev_conved.shape[1]-target_height)//2, 0])
        ...
        W1 = tf.get_variable("w_conv", [filter_size,filter_size,concated.shape[3],num_filters], initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        #print(W1.shape)
        b1 = tf.get_variable("bias", [num_filters], initializer=tf.constant_initializer(0.01))
        #print(b1.shape)

        conv = tf.nn.conv2d(concated, W1, strides=[1,1,1,1], padding="VALID")
...
def conv2d_layer(input, num_filters, filter_size, block_nos):
    with tf.variable_scope(f"block{block_nos}", reuse=tf.AUTO_REUSE):
        W = tf.get_variable("w_conv", [filter_size,filter_size,input.shape[3],num_filters], initializer=tf.truncated_normal_initializer(mean=0, stddev=0.1))
        b = tf.get_variable("bias", [num_filters], initializer=tf.constant_initializer(0.01))

        conv = tf.nn.conv2d(input, filter=W, strides=[1,1,1,1], padding="VALID")
